<template>
 ...
 <h2 v-bind:class="isPromoted && 'promoted'">Promoted Movie</h2>
  ...
  <h2>
    v-for: iterating over an array of strings, iterating over an array of
    objects, iterating over an array of arrays, iterating over an object
  </h2>
 ...
 -->
  ...
  <!-- Form Handling -->
  <h2>
    Capture user inputs, Inputs, Textasreas, Single select dropdown control,
    Multi select control, Checkbox, Checkbox group, Radio, Submit form data
  </h2>
...
into the HTML template wherever it is used. -->
  ...
  <p>
    Computed properties and methods in Vue.js are both used to perform dynamic
    data transformations, but they have some differences in their behavior and
    usage: <br />

    - Computed properties are cached based on their dependencies, which means
    that they are only re-evaluated when their dependencies change. This makes
    them more efficient for frequently used data transformations, since the
    result is cached and only recalculated when necessary. Methods, on the other
    hand, are recalculated every time they are called, regardless of whether
    their dependencies have changed. <br />

    - Computed properties are typically used to transform or derive data from
    other data properties in the component, while methods are used to perform
    more complex logic or data transformations that cannot be easily expressed
    as a simple computed property.<br />

    - Computed properties are defined as functions that return a value, while
    methods are defined as functions that can have side effects and do not need
    to return a value.<br />

    In general, computed properties should be used when you need to transform or
    derive data from other data properties in your component, while methods
    should be used when you need to perform more complex logic or data
    transformations that cannot be easily expressed as a simple computed
    property. Additionally, computed properties are more efficient for
    frequently used data transformations, since their result is cached based on
    their dependencies.
  </p>

  <!-- Computed Properties and v-for  -->
  <h1 class="underline soldout">Computed Properties and v-for</h1>
  <p>
    Conditional List Rendering: using v-for and v-if directive combine with
    template tag
  </p>

  <template v-for="item in items" :key="item.id">
    <h2 v-if="item.price > 100">{{ item.title }} {{ item.price }}</h2></template
  >

  <p>
    Conditional List Rendering: recommended approach is using computed
    properties
  </p>
